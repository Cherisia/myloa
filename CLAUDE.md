...
app/
  ...
  dashboard/
    ...
    _raidHelpers.js      # saveRaid, deleteRaid, saveRaidCompletion, computeAutoRaids, buildAutoRaids, autoSelectNormalRaids, autoSelectExRaid
  ...
  components/
    ...
    AdSense.js / KakaoAdFit.js / SidebarAds.js
    FeedbackButton.js / FeedbackModal.js
  dictionary/
    page.js              # 레이드 보상·직업 시너지 사전 (서버, generateMetadata)
    DictionaryClient.js  # 탭 기반 UI (raids / synergy)
  guide/
    page.js              # 사용 가이드 (index)
    GuideClient.js
    guild/page.js        # 길드 가이드 (index)
    group/page.js        # 그룹 가이드 (index)
  history/
  ...
  auction/
    page.js              # permanentRedirect → /tools/auction
  raids/
    page.js              # permanentRedirect → /dictionary?tab=raids
    RaidRewardClient.js  # 레이드 보상 UI — dictionary/page.js에서 import
  synergy/
    page.js              # permanentRedirect → /dictionary?tab=synergy
    SynergyClient.js     # 직업 시너지 UI — dictionary/page.js에서 import
  admin/
    page.js              # 관리자 전용 (noindex, ADMIN_EMAILS 하드코딩 허용 목록)
    AdminClient.js
  settings/
    page.js              # 서버 컴포넌트 (noindex)
    SettingsClient.js
  ...
  api/
    ...
    characters/          # CRUD + sync(LoA API), check-account, apikey, raids(일정 추가용)
    custom-items/        # CRUD + [id] PATCH·DELETE
    expedition/          # CRUD, join, [id], [id]/members, [id]/favorites, [id]/ranking
    expeditions/[id]/    # GET (그룹 조회용 별도 엔드포인트)
    guild/[id]/schedule/ # 길드 일정 CRUD, [postId], [postId]/join, [postId]/party, [postId]/complete, [postId]/participants/[participantId]
    ...
    history/             # 주간 숙제 히스토리
    feedback/            # 문의/피드백 POST
    cron/                # daily-reset, weekly-reset (Vercel Cron)
    admin/user/[id]/     # 관리자 전용 사용자 조회
lib/
...
| 페이지 | 인덱싱 |
|--------|--------|
| `/`, `/dashboard`, `/dictionary`, `/tools/auction`, `/privacy`, `/guide`, `/guide/guild`, `/guide/group` | ✅ index |
| `/guild`, `/group`, `/settings`, `/history`, `/admin` | ❌ noindex |
| `/raids`, `/synergy` | `permanentRedirect` → `/dictionary?tab=…` |
| `/tools`, `/auction` | `permanentRedirect` → `/tools/auction` |

- noindex 페이지: `robots: { index: false, follow: false }` 명시
- `app/robots.js` disallow 목록 함께 업데이트 (현재: `/api/`, `/settings`, `/guild/`, `/group/`, `/history`, `/admin`)
...
### 반응형 필수 검수
375px / 768px / 1280px / 1536px 4개 뷰포트 정상 동작 필수
