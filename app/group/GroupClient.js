...
function CreateModal({ onClose, onCreated }) {
  ...
  async function submit() {
    if (!name.trim()) { setError('공격대 이름을 입력하세요'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl bg-white dark:bg-[#1e1e1e] shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 pt-5 pb-6 space-y-5">
          <div>
            <h2 className="text-lg ns-extrabold text-gray-900 dark:text-white">공격대 만들기</h2>
      ...
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl bg-white dark:bg-[#1e1e1e] shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 pt-5 pb-6 space-y-5">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">공격대장에게 받은 8자리 코드를 입력하세요</p>
          </div>
          {success ? (
...
export default function GroupClient({ initialGroups }) {
  const router = useRouter()
      ...
      {initialGroups.length === 0 ? (
        <div className="rounded-3xl bg-white dark:bg-[#1e1e1e] shadow-sm px-6 py-16 text-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-sm ns-extrabold text-gray-900 dark:text-white">아직 참여 중인 공격대가 없어요</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">공격대를 만들거나 초대 코드로 참가해보세요</p>
          </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {g.myRole === 'leader' && (
                      <span className="text-[10px] ns-bold bg-[var(--accent-100)] dark:bg-[var(--accent-900)]/30 text-[var(--accent-700)] dark:text-[var(--accent-300)] px-2 py-0.5 rounded-full">공격대장</span>
                    )}
                    {g.myRole === 'officer' && (
                      <span className="text-[10px] ns-bold bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full">부공격대장</span>
                    )}
                    {g.pendingCount > 0 && (
